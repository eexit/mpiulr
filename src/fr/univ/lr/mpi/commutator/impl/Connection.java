...
public class Connection extends Thread implements IConnection {
	...
	public Connection(String callerPhoneNumber) {
		...
		System.out.println("---------Connection built from line : "
		...
		AutoCommutator.getInstance().sendMessage(this.callerPhoneNumber,
				new Message(MessageType.BACKTONE, this.callerPhoneNumber));
	}
	...
	public void receiveEvent(IEvent event) {
		...
		case CALL_TRANSFER_RESPONSE:
			...
			AutoCommutator.getInstance().receiveMessage(
					new Message(MessageType.IS_BUSY, this.callerPhoneNumber,
							this.recipientPhoneNumber));
			break;
		...
		// When the answering machine sends registered message to the caller
		// (answering machine owner)
		case ANSWERING_MESSAGE:
			System.out.println("ANSW_REC CN : " + this.issuerPhoneNumber
					+ " | RN : " + this.recipientPhoneNumber);

			for (Entry<String, String> attr : event.getAttributes().entrySet()) {
				if (attr.getKey().equals(ExchangeAttributeNames.MESSAGE)) {
					AutoCommutator.getInstance().sendMessage(
							this.callerPhoneNumber,
							new Message(MessageType.VOICE_EXCHANGE,
									this.issuerPhoneNumber,
									this.callerPhoneNumber, attr.getValue()));
				}
			}
			break;
		}
	...
	public void receiveMessage(IMessage message) {
		...
		case NUMBERING:
			...
			// System.out.println("Numbering CN : " + this.issuerPhoneNumber +
			// " / RN : " + this.recipientPhoneNumber);

			if (this.recipientPhoneNumber
					.equals(ANSWERING_MACHINE_PHONE_NUMBER)) {
				IEvent answ_pull_event = new Event(
						EventType.ANSWERING_MACHINE_PULL_MESSAGE);
				answ_pull_event.addAttribute(
						ExchangeAttributeNames.CALLER_PHONE_NUMBER,
						this.callerPhoneNumber);
		...
		case RINGING:
			...
			// System.out.println("Ringing CN : "+ this.callerPhoneNumber +
			// " / RN :" + this.recipientPhoneNumber);
			...
			this.timer.schedule(new TimerTask() {
				...
				public void run() {
					...
					AutoCommutator.getInstance().sendMessage(
							...
							new Message(MessageType.STOP_RINGING,
									recipientPhoneNumber));

					// Sends a unavailable recipient event
					IEvent answ_call_event = new Event(
							EventType.UNAVAILABLE_RECIPIENT);
					answ_call_event.addAttribute(
							ExchangeAttributeNames.CALLER_PHONE_NUMBER,
							callerPhoneNumber);
					answ_call_event.addAttribute(
							ExchangeAttributeNames.RECIPIENT_PHONE_NUMBER,
							recipientPhoneNumber);
					answ_call_event.addAttribute(ExchangeAttributeNames.DATE,
			...
			// System.out.println("Ringing CN : "+ this.callerPhoneNumber +
			// " / RN :" + this.calledPhoneNumber);

			// Sends an RINGING message to the recipient
			AutoCommutator.getInstance().sendMessage(
					this.recipientPhoneNumber,
					new Message(MessageType.RINGING, this.callerPhoneNumber,
							this.calledPhoneNumber));
			break;
		...
		case RECIPIENT_PICKUP:
			...
			if (!this.isConnected()) {
				...
				// Sends message CONNECTION_ESTABLISHED to the caller and the
				// recipient
				AutoCommutator.getInstance().sendMessage(
						this.callerPhoneNumber, exchange_message);
				AutoCommutator.getInstance().sendMessage(
						this.calledPhoneNumber, exchange_message);
		...
		case VOICE_EXCHANGE:
			if (this.isConnected()) {
				...
				if (this.issuerPhoneNumber.equals(this.callerPhoneNumber)) {
					AutoCommutator.getInstance().sendMessage(
							this.calledPhoneNumber, message);
				} else {
					AutoCommutator.getInstance().sendMessage(
							this.callerPhoneNumber, message);
				}

			} else if (null != this.calledPhoneNumber) {
				IEvent answ_message = new Event(
						EventType.ANSWERING_MACHINE_PUSH_MESSAGE);
				answ_message.addAttribute(
						ExchangeAttributeNames.CALLER_PHONE_NUMBER,
						this.callerPhoneNumber);
				answ_message.addAttribute(
						ExchangeAttributeNames.RECIPIENT_PHONE_NUMBER,
						this.recipientPhoneNumber);
				answ_message.addAttribute(ExchangeAttributeNames.DATE,
						new Date().toLocaleString());
				answ_message.addAttribute(ExchangeAttributeNames.MESSAGE,
						message.getContent());
		...
		case HANGUP:
			...
			if (this.isConnected()) {
				...
				final Long duration = endTime.getTimeInMillis()
						- startTime.getTimeInMillis() / 1000;

				if (this.issuerPhoneNumber.equals(this.callerPhoneNumber)) {
					System.out.println("Connection killed by the caller");
					IEvent close_connection_event = new Event(
							EventType.CONNECTION_CLOSED);
					close_connection_event.addAttribute(
							ExchangeAttributeNames.CALLER_PHONE_NUMBER,
							callerPhoneNumber);
					close_connection_event.addAttribute(
							ExchangeAttributeNames.RECIPIENT_PHONE_NUMBER,
					...
					close_connection_event.addAttribute(
							...
							duration.toString());
					close_connection_event.addAttribute(
							ExchangeAttributeNames.DATE, new Date()
									.toLocaleString());
					AutoCommutator.getInstance().sendEvent(
							close_connection_event);

					AutoCommutator.getInstance().sendMessage(
							this.callerPhoneNumber,
							new Message(MessageType.CONNECTION_CLOSED,
									this.callerPhoneNumber));
					AutoCommutator.getInstance().sendMessage(
							this.calledPhoneNumber,
							new Message(MessageType.CONNECTION_CLOSED,
					...
					this.timer.schedule(new TimerTask() {
						...
						@Override
						public void run() {
							// Sends the connection duration to the billing
							// service
							IEvent givup_event = new Event(
									EventType.CONNECTION_CLOSED);
							givup_event.addAttribute(
									ExchangeAttributeNames.CALLER_PHONE_NUMBER,
									callerPhoneNumber);
							givup_event
									.addAttribute(
							...
							AutoCommutator.getInstance().receiveEvent(
									kill_event);
				...
				System.out.println(AutoCommutator.getInstance().isConnected(
						calledPhoneNumber));

				if (AutoCommutator.getInstance().isConnected(calledPhoneNumber)) {
					AutoCommutator.getInstance().sendMessage(
							this.callerPhoneNumber,
							new Message(MessageType.CONNECTION_CLOSED,
									this.callerPhoneNumber));
