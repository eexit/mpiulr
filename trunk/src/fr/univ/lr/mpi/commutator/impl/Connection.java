...
public class Connection extends Thread implements IConnection {
	...
	public void receiveEvent(IEvent event) {
		...
		case ANSWERING_MACHINE_WELCOME_MESSAGE:
			AutoCommutator.getInstance().sendMessage(this.callerPhoneNumber, new Message(
				MessageType.VOICE_EXCHANGE, this.recipientPhoneNumber, this.callerPhoneNumber, event.getAttributeValue(ExchangeAttributeNames.MESSAGE)
			));
			break;
	...
	public void receiveMessage(IMessage message) {
			...
			this.timer.schedule(new TimerTask() {
				...
				public void run() {
					...
					AutoCommutator.getInstance().sendMessage(recipientPhoneNumber, new Message(
						MessageType.STOP_RINGING, callerPhoneNumber, recipientPhoneNumber
					));
		...
		case RECIPIENT_PICKUP:
			...
			if (!this.isConnected()) {
				...
				AutoCommutator.getInstance().sendMessage(this.callerPhoneNumber, new Message(
					MessageType.CONNECTION_ESTABLISHED, this.callerPhoneNumber, this.calledPhoneNumber
				));
				AutoCommutator.getInstance().sendMessage(this.calledPhoneNumber, new Message(
					MessageType.CONNECTION_ESTABLISHED, this.calledPhoneNumber, this.callerPhoneNumber
				));
		...
		case VOICE_EXCHANGE:
			...
			} else if (null != this.recipientPhoneNumber) {
		...
		case HANGUP:
			...
			if (this.isConnected()) {
				...
				if (this.issuerPhoneNumber.equals(this.callerPhoneNumber)) {
					...
					System.out.println("---------Connection received a kill message from the caller");
					...
					System.out.println("---------Connection received a kill message from the called");
					...
					this.timer.schedule(new TimerTask() {
						...
						public void run() {
							...
							AutoCommutator.getInstance().sendMessage(callerPhoneNumber, new Message(
								MessageType.CONNECTION_CLOSED, callerPhoneNumber, calledPhoneNumber
							));
				...
				System.out.println("---------Connection received a kill message from the caller while no communication were established");
				...
				if (null != this.timer) {
					...
					AutoCommutator.getInstance().sendMessage(this.recipientPhoneNumber, new Message(
						MessageType.STOP_RINGING, this.callerPhoneNumber, this.recipientPhoneNumber
					));
				}
